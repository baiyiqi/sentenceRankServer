...
stopkey=set([line.strip().decode("utf-8") for line in open('stopword.txt').readlines()])
...
def text_rank_keywords(text):
    ...
    sorted_keywords = sorted(calculated_page_rank.iteritems(), key=operator.itemgetter(1), reverse=True)

    keywords = {sk[0]:sk[1] for sk in sorted_keywords}
...
def text_rank(text, sentobj):
    if text == None or sentobj == None:
        return []
    sents = [(s.sent, s.id) for s in sentobj]
    text = ' '.join(segment(text.decode("utf-8"))).encode("utf-8")
    keywords = text_rank_keywords(text)
    textRank = get_rank(keywords, sents)
    return textRank
...
# text_rank(text, docObj.sentences)